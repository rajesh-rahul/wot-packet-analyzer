<script lang="ts">
    ...
    function findPacketWithReplayTime(replayTime: string): FeedbackMsg {
        try {
            ...
            const needle =
                app_state.gameDurationSecs - (minutes * 60 + seconds - replay.replayStartTime);
    ...
    function findPacketByType(packetType: string, direction: 'before' | 'after'): FeedbackMsg {
        ...
        if (
            !replay.packetSummary.some((it) => it.packetType === packetTypeValidated && it.isShown)
        ) {
    ...
    <div class="flex flex-col gap-1.5">
        ...
        <div class="flex items-center gap-1.5">
            ...
            <Button
                ...
                onclick={() => (feedbackMsg = findPacketByType(packetTypeValue, 'before'))}
                >Before</Button
            >
            <Button
                size="sm"

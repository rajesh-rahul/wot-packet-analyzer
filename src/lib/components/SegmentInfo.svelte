<script lang="ts">
	...
	let focusedSegmentType = $derived.by(() => {
		if (selection_state.state.kind !== 'NoSelection') {
			const startSegType =
				replay.packets[selection_state.state.packetIdx].segments[selection_state.state.startOffset];
			const endSegType =
				replay.packets[selection_state.state.packetIdx].segments[selection_state.state.endOffset];
	...
	function readEntityId(selectionState: SelectionStateType) {
		...
		const [start, end] = getSegmentBoundaries(selectionState.packetIdx, selectionState.startOffset);
...
<div class="flex min-h-[200px] flex-col px-4">
	{#if focusedSegmentType === SegmentType.ZLIB}
		<div class="flex flex-col justify-around gap-1">
			...
			<p class="text-center text-sm tracking-tight text-gray-400">
				Segment contains compressed data. <span class="font-bold">Select Full Segment</span> and
	...
	{:else if focusedSegmentType === SegmentType.PICKLE}
		<div class="flex flex-col justify-around gap-1">
			...
			<p class="text-center text-sm tracking-tight text-gray-400">
				Segment contains a Python Pickle. <span class="font-bold">Select Full Segment</span> and
			...
			<div>
				...
				<div class="flex items-center">
					<span class="mb-[2px] min-w-[60px] pr-2 text-right text-[0.9rem] font-semibold"
						>Name:</span
					>
					<span class="font-mono text-[0.94rem]">{`${vehicleInfo.name}${vehicleInfo.clan}`}</span>
				</div>
		...
		<div class="flex flex-grow flex-col gap-1">
			<div>
				...
				<div class="flex items-center">
					...
					<span class="font-mono text-[0.94rem]">{readEntityId(selection_state.state)}</span>
				</div>
			</div>

			<p class="text-center text-sm tracking-tight text-gray-400">
				A replay contains entities such as Avatar, Vehicle and AreaDestructibles. This ID belongs to
				either an Avatar or AreaDestructibles entity.
			</p>
			<p class="text-center text-sm tracking-tight text-gray-400">
				It may also be a coincidence that this segment matched an actual entity ID in this replay
			</p>
		</div>
	{:else if focusedSegmentType === SegmentType.NORMAL}
		<div class="flex flex-col justify-around gap-1">
			<p class="py-2 text-center text-sm tracking-tight text-gray-400">
				No Segment selected. Different segments are underlined with different colors. Click on any
				cell in a segment for more
			</p>
		</div>
	{/if}
</div>
